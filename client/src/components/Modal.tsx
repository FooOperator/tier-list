...
export const useModal = (which: ModalKey): ModalProps => {
	...
	const { registerModal } = useModalsStore();
	...
	useEffect(() => {
		if (isOpen) thisRef.current?.show();
		else thisRef.current?.close();
	}, [isOpen]);
...
export const Modal = ({
	...
	return (
		<dialog
			className="fixed w-[30rem] h-64 z-50 bg-slate-600 text-white rounded-lg ring-4 ring-slate-900 top-28 p-2"
			ref={modalProps.thisRef}
			open={modalProps.isOpen}>
			<div className="flex basis-[5%] text-slate-200 p-1">
				<button
			...
			<div className="stack basis-full  p-3 flex-col w-full">
				{children}
			</div>
		</dialog>
	);
